...
public class SQLTemplate {
	...
	public static String addColumnSQL(String tableName, List<Field> fieldList){
		String baseSql = "ALTER TABLE " + "`" + toUpcase(tableName) + "`" + " ADD COLUMN (";
	...
	public static String dropColumnSQL(String tableName, Field field) {
		StringBuilder sb = new StringBuilder("ALTER TABLE " + "`" + toUpcase(tableName) + "`" + " DROP COLUMN " + "`" + toLow(field.getFieldName()) + "`");
		return sb.toString();
	}
	...
	public static String addColumnForignKeySQL(String tableName, Field field, String forignInfo){
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " ADD CONSTRAINT fk_" + toUpcase(field.getFieldName()) +  " FOREIGN KEY(");
		sb.append(toUpcase(field.getFieldName()) + ") REFERENCES " + toLow(forignInfo));
		return sb.toString();
	}
	...
	public static String dropColumnForignKeySQL(String tableName, Field field){
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " DROP CONSTRAINT fk_" + toUpcase(field.getFieldName()));
		return sb.toString();
	}
...
//	}
	...
	public static String addColumnPrimaryKeySQL(String tableName, Field field) {
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " ADD CONSTRAINT pk_" + toUpcase(field.getFieldName())+" PRIMARY KEY (" + toUpcase(field.getFieldName()) + ")");
		return sb.toString();
	}
	...
	public static String dropColumnPrimaryKeySQL(String tableName, Field field) {
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " DROP CONSTRAINT pk_" + toUpcase(field.getFieldName()));
		return sb.toString();
	}
	...
	public static String addColumnDefaultSQL(String tableName, Field field, String defaultInfo) {
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " ALTER COLUMN " + toUpcase(field.getFieldName()) + " SET DEFAULT '" + defaultInfo + "'");
		return sb.toString();
	}
	...
	public static String dropColumnDefaultSQL(String tableName, Field field) {
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " ALTER COLUMN " + toUpcase(field.getFieldName()) + " DROP DEFAULT");
		return sb.toString();
	}
	...
	public static String addColumnUniqueSQL(String tableName, Field field) {
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " ADD CONSTRAINT un_" + toUpcase(field.getFieldName())+" UNIQUE(" + toUpcase(field.getFieldName()) + ")");
		return sb.toString();
	}
	...
	public static String dropColumnUniqueSQL(String tableName, Field field) {
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " DROP CONSTRAINT un_" + toUpcase(field.getFieldName()));
		return sb.toString();
	}
	...
	public static String addColumnNotNullSQL(String tableName, Field field) {
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " ALTER COLUMN " + toUpcase(field.getFieldName()) + " NOT NULL");
		return sb.toString();
	}
	...
	public static String dropColumnNotNullSQL(String tableName, Field field) {
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " ALTER COLUMN " + toUpcase(field.getFieldName()) + " NULL");
		return sb.toString();
	}
	...
	public static String alterColumnSQL(String tableName, Field field) {
		StringBuilder sb = new StringBuilder("ALTER TABLE " + toLow(tableName) + " ALTER COLUMN " + toUpcase(field.getFieldName()) + " " + field.getFieldType() + checkFieldTypeCount(field.getFieldTypeCount()));
		return sb.toString();
	}
	...
	public static String createTableSQL(Entity entity){
		...
		List<Field> fields = entity.getFields();//全部字段
		...
		for (int i = 0; i < fieldCount;i++) {
			...
			String fieldName = toUpcase(field.getFieldName());
		...
		if (pSize > 0) {
			...
			for (int i = 0; i < pSize; i++) {
				primaryKeySb.append(toUpcase(primaryFieldList.get(i).getFieldName()));
				if (i != pSize - 1) {
					primaryKeySb.append(",");
				} else {
					primaryKeySb.append(")");
		...
		if (uSize > 0) {
			...
			for (int i = 0; i < uSize; i++) {
				uniqueSb.append(toUpcase(uniqueFieldList.get(i).getFieldName()));
				if (i != uSize - 1) {
					uniqueSb.append(",");
				} else {
					uniqueSb.append(")");
		...
		if (fSize > 0) {
			for (int i = 0; i < fSize; i++) {
				...
				foreignSb.append(toUpcase(field.getForignCondition()));
...
//		}
        ...
        //引擎 默认字符集  自增起始位 ENGINE=InnoDB DEFAULT CHARSET=utf8 AUTO_INCREMENT=1
	...
	private static String toUpcase(String s) {
		return StringCaseUtil.upcaseAll(s);
	}
